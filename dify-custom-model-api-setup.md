...
| 场景 | 推荐模型 | 理由 |
|:---|:---|:---|
| 智能助手（对话） | 从当前目录选择适合对话的模型 | 用真实业务问题验证回答、上下文和流式输出 |
| 知识库问答（RAG） | 从当前模型目录选择适合中文与长文本的 LLM | 先用真实知识库问题验证召回后回答 |
| 文本分类/摘要 | 从当前目录选择低延迟文本模型 | 用自己的样本检查准确率和输出格式 |
| 工作流 Agent | 选择当前支持工具调用的模型 | 必须验证 tools、流式事件和工具结果回传 |
...
| Agent 工具调用失败 | 模型或供应商插件的 tools 协议不兼容 | 先保留原始错误，再核对模型能力和工具结果回传 |
