...
新站统一使用美元（USD）充值并按 1:1 计入账户余额：

- **⭐️ 1 AIFast Credit = US$1**；
- 支付方式、折扣、支付网络和最终到账以控制台与结账页当前显示为准；
- ⭐️ AIFast Credits 是平台余额与模型计价的使用单位标识，不是独立货币或加密货币代币；
- 信用卡支付可能涉及通道或发卡行费用；使用其他支付方式前必须核对币种、网络、地址和到账规则。
...
通常不需要自行配置海外代理。Claude、GPT、Gemini等国外模型支持国内直连；正式部署仍应从目标地区和运营商测试 DNS、TLS、首包时间、流式输出和错误率。
